...
import com.wenhuabin.database.DataSupport;
import com.wenhuabin.database.MydatePickerDialog;
...
public class Showmain extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.summain);
		init();
		datainsert();
		showdata();
		alarmsave();
	...
	private void init() {
		...
		detail_data_app.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent it = new Intent(Showmain.this, NetTraffic.class);
				startActivity(it);
			}
		});
	...
	private void search_result() {
		...
		Cursor t3gshutdown = minsert.selectbetweenday(TX3G, SHUTDOWN,
				startdate, overdate);
		liuliangzongbiaoti.setText(startdate + "��" + overdate + "������");
		if (r3gst.moveToNext()) {
